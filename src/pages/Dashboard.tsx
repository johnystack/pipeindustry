    ...
    const fetchData = async () => {
      if (user) {
        ...
        if (investmentsError) {
          ...
          const activeInvestments = investmentsData?.filter((investment) => investment.status === 'active') || [];

          const totalBalance = activeInvestments.reduce((acc, investment) => acc + investment.amount, 0);
        ...
        if (profileError) {
          ...
          setStatsData((prev: any) => ({ ...prev, withdrawableBalance: profileData.withdrawable_balance, referralEarnings: profileData.referral_earnings }));
        }
                    ...
                    return (
                      <div key={index} className="flex items-center justify-between p-4 rounded-lg border bg-background/50">
                        ...
                        <div className="text-right">
                          ...
                          <Link to={`/manage-investment/${investment.id}`} state={{ investment, withdrawableBalance: statsData.withdrawableBalance }}>
                    ...
                    <div key={index} className="flex items-center justify-between p-4 rounded-lg border bg-background/50">
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-full ${
                          ...
                          {transaction.type === 'Investment' && <TrendingUp className="h-4 w-4 text-primary" />}
                          {transaction.type === 'Withdrawal' && <ArrowDownRight className="h-4 w-4 text-warning" />}
                          {transaction.type === 'Referral' && <Users className="h-4 w-4 text-success" />}
                        </div>
                        <div>
                      ...
                      <div className="text-right">
                        <div className={`font-semibold ${
                          transaction.amount.toString().startsWith('+') ? 'text-success' : 'text-warning'
                        }`}>
                          {transaction.amount}
                        </div>
