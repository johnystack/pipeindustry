    ...
    const fetchReferralData = async () => {
      if (user) {
        ...
        const { data: referredUsers, error: referredUsersError } = await supabase
          .from("profiles")
          .select("id, first_name, last_name, created_at, has_invested") // Assuming has_invested column
        ...
        const activeReferrals = referredUsers.filter(u => u.has_invested).length; // Assuming has_invested

        // Fetch referral earnings from transactions
        const { data: referralTransactions, error: referralTransactionsError } = await supabase
          .from("transactions")
          .select("amount")
          .eq("user_id", user.id)
          .eq("type", "Referral")
        ...
        if (referralTransactionsError) {
          console.error("Error fetching referral transactions:", referralTransactionsError);
          return;
        }

        const totalEarned = referralTransactions.reduce((acc, t) => acc + t.amount, 0);

        setReferralStats({
          totalReferrals,
          activeReferrals,
          totalEarned,
          currentLevel: "Bronze", // Placeholder, actual level logic might be more complex
        });

        setRecentReferrals(referredUsers.map(u => ({
          name: `${u.first_name} ${u.last_name}`,
          email: "", // Email not available from profiles table directly
          joinDate: new Date(u.created_at).toLocaleDateString(),
          status: u.has_invested ? "Active" : "Pending",
          invested: "$0", // Not directly available from profiles table
          commission: "$0", // Not directly available from profiles table
        })));
      }
    };
    ...
    <div className="container mx-auto p-6 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card className="bg-gradient-card border-border shadow-card">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{referralStats.currentLevel}</div>
              <p className="text-xs text-muted-foreground">
                Your referral tier
              </p>
            </CardContent>
        ...
        <Card className="bg-gradient-card border-border shadow-card">
          <CardHeader>
            ...
            <CardDescription>
              Share this link with friends and earn commissions on their investments
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex gap-2">
              <Input 
                value={`https://pipindustry.com/ref/${user?.email}`} 
                readOnly 
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {commissionStructure.map((level, index) => (
                <div key={index} className="text-center space-y-3">
                  <div className={`w-16 h-16 ${level.color} rounded-full flex items-center justify-center mx-auto`}>
              ...
              <CardContent>
                <div className="space-y-4">
                  {recentReferrals.map((referral) => (
                    <div key={referral.id} className="flex items-center justify-between p-4 rounded-lg border bg-background/50">
                      ...
                      <div className="text-right space-y-1">
                        ...
                        <div className="text-sm">
                          ...
                          <div className="text-success">Earned: {referral.commission}</div>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <CardContent>
                <div className="space-y-4">
                  {commissionEarnings.map((earning) => (
                    <div key={earning.id} className="flex items-center justify-between p-4 rounded-lg border bg-background/50">
                      <div>
                        <h4 className="font-medium">{earning.description}</h4>
                        <p className="text-sm text-muted-foreground">{earning.reference}</p>
                        <p className="text-xs text-muted-foreground">{new Date(earning.created_at).toLocaleDateString()}</p>
                      </div>
                      <div className="text-success font-semibold">+${earning.amount.toLocaleString()}</div>
                    </div>
                  ))}
                </div>
              </CardContent>
