    ...
    <div className="container mx-auto p-6 space-y-8">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Withdraw your earnings to your personal cryptocurrency wallet. All withdrawals are processed within 24 hours.
          </p>
        </div>
                          ...
                          <SelectItem key={crypto.id} value={crypto.id}>
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-muted-foreground">
                                ({(withdrawableBalance * getConversionRate(crypto.symbol)).toFixed(6)} available)
                              </span>
                  ...
                  <div className="space-y-2">
                    ...
                    {selectedCryptoData && (
                      <div className="flex justify-between text-xs text-muted-foreground">
                        <span>Available: {(withdrawableBalance * getConversionRate(selectedCryptoData.symbol)).toFixed(6)} {selectedCryptoData.symbol}</span>
                        
                        <Button
                          variant="link"
                          size="sm"
                          className="h-auto p-0 text-xs"
                          onClick={() => setAmount((withdrawableBalance * getConversionRate(selectedCryptoData.symbol)).toFixed(6))}
                        >
                          Use Max
                        </Button>
                      </div>
                  ...
                  {selectedCryptoData && (
                    <div className="space-y-3 p-4 bg-muted/50 rounded-lg">
                      <div className="flex justify-between text-sm">
                        <span>Network:</span>
                        <Badge variant="outline">{selectedCryptoData.network}</Badge>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>Withdrawal Fee:</span>
                        <span>{selectedCryptoData.fee} {selectedCryptoData.symbol}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>You will receive:</span>
                        <span className="font-medium">{receiveAmount} {selectedCryptoData.symbol}</span>
                      </div>
                    </div>
                  )}
                      ...
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="space-y-2">
                          ...
                          <div className="space-y-1 p-3 bg-muted rounded-lg text-sm">
                            <div><strong>Amount:</strong> {amount} {selectedCryptoData?.symbol}</div>
                            <div><strong>Network Fee:</strong> {selectedCryptoData?.fee} {selectedCryptoData?.symbol}</div>
                            <div><strong>You Receive:</strong> {receiveAmount} {selectedCryptoData?.symbol}</div>
                            <div><strong>Address:</strong> {address.slice(0, 20)}...</div>
                          </div>
                          <div className="text-warning">
                            This action cannot be undone. Please ensure the address is correct.
                          </div>
              ...
              <Card className="crypto-card">
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between p-4 border border-border rounded-lg bg-background/50">
                    <div>
                      <div className="font-medium">Total Withdrawable Balance</div>
                      <div className="text-2xl font-bold text-primary">
                        ${(withdrawableBalance || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </div>
                    </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  {withdrawalHistory.map((withdrawal) => (
                    <div key={withdrawal.id} className="flex items-center justify-between p-4 border border-border rounded-lg bg-background/50">
                      ...
                      <div className="text-right">
                        <Badge 
                          variant="outline" 
                          className={
                            withdrawal.status === 'completed' ? 'status-completed' :
                            withdrawal.status === 'pending' ? 'status-pending' : 'status-active'
                          }
                        >
                          {withdrawal.status}
                        </Badge>
