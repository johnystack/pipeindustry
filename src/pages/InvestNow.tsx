...
import { Copy, QrCode, Clock, CheckCircle, AlertCircle, Star, Crown, Diamond } from "lucide-react";
      ...
      .insert([
        {
          ...
          duration: duration,
          due_date: new Date(new Date().getTime() + duration * 24 * 60 * 60 * 1000),
        },
      ]);
            ...
            <div className="grid lg:grid-cols-2 gap-8">
              ...
              <Card className="crypto-card">
                ...
                <CardContent className="space-y-6">
                  ...
                  <Button onClick={handleInvest} className="w-full">Invest</Button>
                  ...
                  {selectedPlanData && selectedCryptoData && (
                    <div className="space-y-4 p-4 bg-muted/50 rounded-lg">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm">{selectedCryptoData?.minDeposit} {selectedCryptoData?.symbol}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium">Confirmations:</span>
                        <span className="text-sm">{selectedCryptoData?.confirmations}</span>
                      </div>
                    </div>
                  )}
                </CardContent>
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="space-y-3 p-4 bg-warning/10 border border-warning/20 rounded-lg">
                      ...
                      <ul className="text-xs space-y-1 text-muted-foreground">
                        <li>• Only send {selectedCryptoData?.name} to this address</li>
                        <li>• Minimum deposit: {selectedCryptoData?.minDeposit} {selectedCryptoData?.symbol}</li>
                        <li>• Requires {selectedCryptoData?.confirmations} network confirmations</li>
              ...
              <CardContent>
                <div className="space-y-4">
                  {recentDeposits.map((deposit) => (
                    <div key={deposit.id} className="flex items-center justify-between p-4 border border-border rounded-lg bg-background/50">
                      ...
                      <div className="text-right">
                        <Badge 
                          variant="outline" 
                          className={deposit.status === 'completed' ? 'status-completed' : 'status-pending'}
                        >
                          {deposit.status}
                        </Badge>
