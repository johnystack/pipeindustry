...
const UserInvestmentDetails = () => {
  ...
  const completedInvestments = investments.filter(
    (i) => i.status === "completed" || i.status === "withdrawn"
  );

  // 1. What they invested
  const activeInvested = activeInvestments.reduce((sum, i) => sum + (Number(i.amount) || 0), 0);
  const lifetimeInvested = investments
    .filter((i) => ["active", "completed", "withdrawn"].includes(i.status))
    .reduce((sum, i) => sum + (Number(i.amount) || 0), 0);

  // 2. How much withdrawn
  const totalClaimed = investments.reduce((sum, i) => sum + (Number(i.claimed_amount) || 0), 0);
  const completedBankWithdrawn = withdrawals
    .filter((w) => w.status === "completed")
    .reduce((sum, w) => sum + (Number(w.amount) || 0), 0);
  const pendingBankWithdrawn = withdrawals
    .filter((w) => w.status === "pending")
    .reduce((sum, w) => sum + (Number(w.amount) || 0), 0);

  // 3. Left on plan execution
  const totalActiveTarget = activeInvestments.reduce((sum, i) => sum + (Number(i.amount) || 0) * 1.5, 0);
  const totalActiveClaimed = activeInvestments.reduce((sum, i) => sum + (Number(i.claimed_amount) || 0), 0);
  const totalLeftOnPlan = Math.max(0, totalActiveTarget - totalActiveClaimed);
  const overallProgress = totalActiveTarget > 0 
    ? Math.min(100, Math.round((totalActiveClaimed / totalActiveTarget) * 100)) 
    : (activeInvestments.length === 0 && completedInvestments.length > 0 ? 100 : 0);
    ...
    <div className="container mx-auto p-4 md:p-8 space-y-10 animate-in fade-in duration-700">
          ...
          <div>
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-2">
                ...
                {activeInvestments.length > 0 ? (
                  <Badge className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 font-black px-3 py-0.5 rounded-lg text-[10px] tracking-widest">
                    ACTIVE INVESTOR
                  </Badge>
                ) : (
                  <Badge variant="outline" className="border-white/10 text-muted-foreground font-black px-3 py-0.5 rounded-lg text-[10px] tracking-widest">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="bg-slate-900/40 border-2 border-white/5 rounded-[2rem] overflow-hidden backdrop-blur-xl">
            <CardHeader className="pb-2">
                <CardTitle className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-50 flex items-center justify-between">
                    <span>Invested Capital</span>
                    <TrendingUp className="h-3 w-3 text-primary" />
                </CardTitle>
            </CardHeader>
            <CardContent className="space-y-1">
                <div className="text-3xl font-black text-white tracking-tighter">₦{activeInvested.toLocaleString()}</div>
                <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">
                    {activeInvestments.length > 0 ? `${activeInvestments.length} Active Positions` : (lifetimeInvested > 0 ? `Total: ₦${lifetimeInvested.toLocaleString()}` : "No Active Deposits")}
                </p>
            </CardContent>
        </Card>
        ...
        <Card className="bg-slate-900/40 border-2 border-white/5 rounded-[2rem] overflow-hidden backdrop-blur-xl">
            ...
            <CardContent className="space-y-1">
                <div className="text-3xl font-black text-emerald-500 tracking-tighter">₦{totalClaimed.toLocaleString()}</div>
                <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">
                    Bank Paid: ₦{completedBankWithdrawn.toLocaleString()}
        ...
        <Card className="bg-slate-900/40 border-2 border-white/5 rounded-[2rem] overflow-hidden backdrop-blur-xl">
            <CardHeader className="pb-2">
                <CardTitle className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-50 flex items-center justify-between">
                    <span>Left On Execution</span>
                    <Clock className="h-3 w-3 text-amber-500" />
                </CardTitle>
            </CardHeader>
            <CardContent className="space-y-1">
                <div className="text-3xl font-black text-amber-500 tracking-tighter">₦{totalLeftOnPlan.toLocaleString()}</div>
                <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">
                    {activeInvestments.length > 0 ? `${overallProgress}% executed` : (completedInvestments.length > 0 ? "100% Executed" : "No Active Plans")}
                </p>
            </CardContent>
        </Card>
        ...
        <Card className="bg-slate-900/40 border-2 border-white/5 rounded-[2rem] overflow-hidden backdrop-blur-xl">
            ...
            <CardContent className="space-y-1">
                <div className="text-3xl font-black text-cyan-400 tracking-tighter">₦{user.withdrawable_balance?.toLocaleString() || "0.00"}</div>
                <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">
      ...
      <div className="space-y-8">
        <h2 className="text-2xl font-black flex items-center gap-3 tracking-tight">
            <Briefcase className="h-7 w-7 text-primary" /> TRADING INVENTORY ({investments.length})
        </h2>
                ...
                <div className="p-8 md:p-10 flex flex-col lg:flex-row justify-between gap-10 items-stretch lg:items-center">
                  <div className="flex-1 space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="p-5 bg-slate-950/50 rounded-2xl border border-white/5 text-center sm:text-left">
                        <p className="text-[9px] text-muted-foreground font-black mb-1 uppercase tracking-widest opacity-50">Allocated Capital</p>
                        <p className="text-xl font-black tracking-tighter">₦{inv.amount.toLocaleString()}</p>
                      </div>
                      <div className="p-5 bg-slate-950/50 rounded-2xl border border-white/5 text-center sm:text-left">
                        <p className="text-[9px] text-muted-foreground font-black mb-1 uppercase tracking-widest opacity-50">Target Return (150%)</p>
                        <p className="text-xl font-black text-emerald-500 tracking-tighter">₦{targetReturn.toLocaleString()}</p>
                      </div>
                      <div className="p-5 bg-slate-950/50 rounded-2xl border border-white/5 text-center sm:text-left">
                        <p className="text-[9px] text-muted-foreground font-black mb-1 uppercase tracking-widest opacity-50">Withdrawn / Claimed</p>
                        <p className="text-xl font-black text-purple-400 tracking-tighter">₦{claimed.toLocaleString()}</p>
                      </div>
                      <div className="p-5 bg-slate-950/50 rounded-2xl border border-white/5 text-center sm:text-left">
                        <p className="text-[9px] text-muted-foreground font-black mb-1 uppercase tracking-widest opacity-50">Left On Execution</p>
                        <p className="text-xl font-black text-amber-400 tracking-tighter">₦{leftOnPlan.toLocaleString()}</p>
                      </div>
                    </div>

                    {/* Execution Progress for Active Plans */}
                    {inv.status === 'active' && (
